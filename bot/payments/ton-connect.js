  ...
  try {
    ...
    if (matchingTx) {
      ...
      try {
        ...
        console.error('[TON] Ошибка получения имени клиента:', err);
      }
        ...
        if (TELEGRAM_ADMIN_ID) {
          await bot.sendMessage(
            ...
            `💰 <b>Новая оплата!</b>\n\n` +
              ...
              `💵 <b>Сумма:</b> ${planData.price_ton} TON\n` +
              `📅 <b>Продлено на:</b> ${planData.days} дней\n` +
              `🕐 <b>Время:</b> ${new Date().toLocaleString('ru-RU')}\n` +
              `🔗 <b>TX:</b> <code>${matchingTx.transaction_id.hash}</code>`,
            { parse_mode: 'HTML' }
          );
        }
      } catch (err) {
        console.error('[TON] Ошибка отправки уведомления админу:', err);
      }
