...
class APIClient {
  ...
  /**
   * Запустить диагностику ТСПУ для сервера
   * @param {string} ip - IP адрес сервера (опционально)
   * @returns {Promise<Object>} - Результат диагностики
   */
  async checkTSPU(ip = null) {
    return this.request("/api/tspu/check", "POST", ip ? { ip } : {});
  }
